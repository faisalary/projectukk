    ...
    <div class="row">
        <div class="col-auto my-auto pe-1">
            <div class="form-check form-check-primary">
                <input name="selected_logbook" class="form-check-input" type="radio" value="{{ $item->id_logbook_week }}" id="logbook_{{ $key }}" {{ ((isset($checked) && $checked == $item->id_logbook_week) ?: $loop->first) ? 'checked' : '' }}>
            </div>
        </div>
        <div class="col ps-0 d-flex justify-content-between">
            <div class="d-flex flex-column align-items-start">
                <h6 class="mb-1">Minggu Ke {{ $item->week }}</h6>
