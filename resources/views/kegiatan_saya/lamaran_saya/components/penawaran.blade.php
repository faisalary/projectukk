@if ($pendaftar->where('konfirmasi_status', 3)->count() == 0)
    ...
    @foreach ($pendaftar->where('konfirmasi_status', 3) as $item)
        <div class="card mt-2">
            <div class="card-body">
                ...
                <a href="{{ url('kegiatan-saya/detail', $item->id_lowongan) }}"
                    style="color:#4B4B4B">
                    <div class="row mb-2">
                        <div class="col-2">
                ...
                <div class="text-left">
                    <button type="button" data-id="{{ $item->nim }}"
                        data-lowongan="{{ $item->lowongan_magang->id_lowongan }}"
                        onclick="ambil($(this))" class="btn btn-success waves-effect me-2"
                        data-bs-toggle="modal">Ambil Tawaran
                    </button>
                    <button type="button" data-id="{{ $item->nim }}"
                        data-lowongan="{{ $item->lowongan_magang->id_lowongan }}"
                        onclick="tolak($(this))" class="btn btn-danger waves-effect"
                        data-bs-toggle="modal">Tolak Tawaran
                    </button>
                </div>
                <hr />
                <div class="row mt-2">
                    <div class="col-12 d-flex justify-content-between">
                        <div class="col-6">
                            ...
                            @if (empty($item->lowongan_magang->nominal_salary))
                                <span> <i class="ti ti-currency-dollar ms-3"
                                        style="font-size: medium;"></i> Tidak
                                    Berbayar</span>
                            @else
                                <span> <i class="ti ti-currency-dollar ms-3"
                        ...
                        <div class="col-6">
                            <div class="text-end"
                                style="font-size: medium; color : #4EA971">
                                Lamaran terkirim pada
                                {{ $item->tanggaldaftar?->format('d F Y') }}</div>
                        </div>
                    </div>
                </div>
            </div>
