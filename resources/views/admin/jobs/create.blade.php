            ...
            <div class="card">
                <div class="card-body">
                    ...
                    @elseif(count($industries) == 0)
                        <div class="alert alert-danger alert-dismissible fade show">
                            <h4 class="alert-heading"><i class="fa fa-warning"></i> Job Industry Empty!</h4>
                            <p>You do not have any Job Industry created. You need to create the Job Industry first to add
                                the job .
                                <a href="{{ route('admin.job-industries.create') }}" class="btn btn-info btn-sm m-l-15"
                                   style="text-decoration: none;"><i
                                            class="fa fa-plus-circle"></i> @lang('app.createNew') @lang('menu.jobIndustries')
                                </a>
                            </p>
                        </div>
                    @elseif(count($categories) == 0)
                        <div class="alert alert-danger alert-dismissible fade show">
                            ...
                            <p>You do not have any Job Category created. You need to create the Job Category first to add
                                the job .
                        ...
                        <form class="ajax-form" method="POST" id="createForm">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                        <label for="address">@lang('menu.jobIndustries')</label>
                                        <select name="industry_id" id="industry_id"
                                                class="form-control select2">
                                            <option value="">@lang('app.choose') @lang('menu.jobIndustries')</option>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group" id="totalPositions">
                                        <label for="address">
                                            @lang('modules.jobs.totalPositions')
                                            <div id="totalPosDiff" style="display:none">
                                                &nbsp; &nbsp; &nbsp;
                                                <div class="icheckbox_flat-green" aria-checked="false" aria-disabled="false" style="position: relative;">
                                                    <input type="checkbox" name="is_total_pos_diff" id="cbTotalPosDiff" class="flat-red"  style="position: absolute; opacity: 0;">
                                                    <ins class="iCheck-helper" style="position: absolute; top: 0%; left: 0%; display: block; width: 100%; height: 100%; margin: 0px; padding: 0px; background: rgb(255, 255, 255); border: 0px; opacity: 0;"></ins>
                                                </div>
                                                Different Total Positions for each Locations
                                            </div>
                                        </label>
                                        <input type="number" class="form-control" name="total_positions[0]"
                                               id="total_positions">
                                    </div>
...
@push('footer-script')
    ...
    <script>
        ...
        $('#location_id').change(function () {
            $('#totalPosDiff').css('display', $(this).val().length < 2 ? 'none' : 'inline');
            $('#cbTotalPosDiff').iCheck('uncheck');
        });

        $('#cbTotalPosDiff').on('ifChecked', function(){
            var locs = $('#location_id').children();
            $('#total_positions').attr('placeholder', 'Total Positions in ' + locs.eq(0).text());
            for (i=1;i<locs.length;i++) {
                $('#totalPositions').append('<input type="number" class="form-control total-pos" name="total_positions[' + i + ']" placeholder="Total Positions in ' + locs.eq(i).text() + '">');
            }
        });

        $('#cbTotalPosDiff').on('ifUnchecked', function(){
            $('.total-pos').remove();
            $('#total_positions').attr('placeholder', '');
        });
