...
@section('content')
    <div id="tabs" style="display: flex; flex-direction: column;  gap:1rem;">
        <div id="aside-profile-dosen" class="list-group py-3 d-flex flex-row gap-2 nav nav-pills align-items-center px-3"
            style="height: max-content; width: max-content;" role="tablist" aria-orientation="vertical">
            <button id="v-pills-informasi-pribadi-tab"
                class="border-none rounded d-flex flex-row align-items-center justify-content-around nav-link active"
                data-bs-toggle="pill" data-bs-target="#v-pills-informasi-pribadi" type="button" role="tab"
                aria-controls="v-pills-informasi-pribadi" aria-selected="true" style="width: 14rem; height: 3rem;">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M16 6H19C19.5523 6 20 6.44772 20 7V18C20 19.1046 19.1046 20 18 20C16.8954 20 16 19.1046 16 18V5C16 4.44772 15.5523 4 15 4H5C4.44772 4 4 4.44772 4 5V17C4 18.6569 5.34315 20 7 20H18"
                    ...
                    <path d="M8 8H12" stroke="white" stroke-opacity="0.2" stroke-width="1.5" stroke-linecap="round"
                        stroke-linejoin="round" />
                    <path d="M8 12H12" stroke="#4B465C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                    <path d="M8 12H12" stroke="white" stroke-opacity="0.2" stroke-width="1.5" stroke-linecap="round"
                        stroke-linejoin="round" />
                    <path d="M8 16H12" stroke="#4B465C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                    <path d="M8 16H12" stroke="white" stroke-opacity="0.2" stroke-width="1.5" stroke-linecap="round"
                        stroke-linejoin="round" />
                </svg>
                <span>Informasi Pribadi</span>
            </button>
            <button id="v-pills-ubah-password-tab"
                class="border-none rounded d-flex flex-row align-items-center justify-content-around nav-link"
                data-bs-toggle="pill" data-bs-target="#v-pills-ubah-password" type="button" role="tab"
                aria-controls="v-pills-ubah-password" aria-selected="false" style="width: 14rem; height: 3rem;">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <circle cx="8" cy="15" r="4" stroke="#4B465C" stroke-width="1.5" stroke-linecap="round"
                        stroke-linejoin="round" />
                    <path d="M10.8496 12.15L18.9996 4" stroke="#4B465C" stroke-width="1.5" stroke-linecap="round"
                        stroke-linejoin="round" />
                    <path d="M18 5L20 7" stroke="#4B465C" stroke-width="1.5" stroke-linecap="round"
                        stroke-linejoin="round" />
                    <path d="M15 8L17 10" stroke="#4B465C" stroke-width="1.5" stroke-linecap="round"
                        stroke-linejoin="round" />
                </svg>
                        ...
                        error: function(xhr, status, error) {
                            ...
                            if (res.errors) {
                                $.each(res.errors, function(key, value) {
                                    if ($('#container-change-image').find(
                                            `[name="${key}"]`).length > 0) {
                                        $('#container-change-image').find(
                                            `[name="${key}"]`).addClass(
                                            'is-invalid');
                                        $('#container-change-image').find(
                                                `[name="${key}"]`).parents(
                                                '.form-group').find('.invalid-feedback')
                                            .html(value[0]).addClass('d-block');
                                    }
                                });
                            } else {
                                showSweetAlert({
