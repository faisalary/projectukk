...
class AdminJobOnboardController extends AdminBaseController
{
    ...
    public function store(StoreRequest $request)
    {
        ...
        $onBoard->salary_offered = str_replace(',','', $request->salary);
    ...
    public function data(Request $request)
    {
        ...
        $jobApplications = Onboard::select('job_application_id', 'on_board_details.id','job_applications.id as application_id', 'jobs.title', 'job_locations.location','on_board_details.joining_date','on_board_details.accept_last_date', 'on_board_details.hired_status')
            ...
            ->with(['applications:id,job_id,user_id', 'applications.user:id,name']);
        ...
        return DataTables::of($jobApplications)
            ...
            ->addColumn('full_name', function ($row) {
                return '<a href="javascript:;" class="show-detail" data-widget="control-sidebar" data-slide="true" data-row-id="' .  $row->application_id . '">' . ucwords($row->applications->user->name) . '</a>';
            })
    ...
    public function sendOffer($userID){
        $jobApplication = JobApplication::select('id', 'job_id')->with(['user:name,email', 'job:id,title', 'onboard:id,offer_code,job_application_id,hired_status'])->where('id', $userID)->first();
