...
class InterviewScheduleController extends AdminBaseController
{
    ...
    public function data(Request $request){
        ...
        $shedule = InterviewSchedule::select('interview_schedules.id','interview_schedules.status', 'interview_schedules.schedule_date', 'job_application_id')
            ->leftjoin('job_applications', 'job_applications.id', 'interview_schedules.job_application_id')
            ->with(['jobApplication:id,job_id,user_id', 'jobApplication.user:id,name']);
        // Filter by status
        ...
        return DataTables::of($shedule)
            ...
            ->addColumn('full_name', function ($row) {
                return ucwords($row->jobApplication->user->name);
            })
    ...
    public function update(UpdateRequest $request, $id){
        ...
        $interviewSchedule = InterviewSchedule::select('id', 'job_application_id', 'schedule_date', 'status')
                            ->with([
                                'jobApplication:id,user_id,job_id,status_id',
                                'jobApplication.user:id,name',
                                'employees',
                                'comments'
                            ])
                            ->where('id', $id)->first();
    ...
    public function notify($id, $type){
        ...
        if($type == 'notify'){
            ...
            Notification::send($jobApplication, new CandidateReminder($jobApplication, $jobApplication->schedule));
