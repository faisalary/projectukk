...
'Native / Bilingual Proficiency', 'Professional Working Proficiency', 'Limited Working Proficiency', 'Elementary
Proficiency'
];
...
for ($i=0;$i<count($universities);$i++){ if (trim($universities[$i]) !='' && trim($degrees[$i]) !='' &&
    trim($studies[$i]) !='' && trim($start_dates[$i]) !='' && trim($gpas[$i]) !='' && trim($is_graduateds[$i]) !='' ){
    $education=new ProfileUserEducation(); if($request->has('education_id')){
    if($i < count($education_ids)){ if($education_ids[$i] !=null)
        $education=ProfileUserEducation::find($education_ids[$i]); } } $education->profile_user_id = $profile->id;
        ...
        for ($i=0;$i<count($skills);$i++){ if (trim($skills[$i]) !='' ){ $skill=new Skill();
            ...
            for ($i=0;$i<count($start_periods);$i++) { if (trim($start_periods[$i]) !='' && trim($comps[$i]) !='' &&
                trim($positions[$i]) !='' ) { $jobExp=new ProfileUserExperience(); if($request->has('experience_id')){
                if($i < count($experience_ids)){ if($experience_ids[$i] !=null)
                    $jobExp=ProfileUserExperience::find($experience_ids[$i]); } } $jobExp->profile_user_id =
                    $profile->id;
                    ...
                    for ($i=0;$i<count($language_names);$i++) { if (trim($language_names[$i]) !='' &&
                        trim($language_levels[$i]) !='' ) { $lang=new ProfileUserLanguage(); if($request->
                        has('language_id')){
                        if($i < count($language_ids)){ if($language_ids[$i] !=null)
                            $lang=ProfileUserLanguage::find($language_ids[$i]); } } $lang->profile_user_id =
                            $profile->id;
                            ...
                            for ($i=0;$i<count($portfolios);$i++) { if (trim($portfolios[$i]) !='' && trim($urls[$i])
                                !='' ) { $port=new ProfileUserPortfolio(); if($request->has('portfolio_id')){
                                if($i < count($portfolio_ids)){ if($portfolio_ids[$i] !=null)
                                    $port=ProfileUserPortfolio::find($portfolio_ids[$i]); } } $port->profile_user_id =
                                    $profile->id;
                                    ...
                                    // Files::deleteFile($profile->resumeDocument->hashname,
                                    'documents/users'.$profile->id);
                                    $hashname = Files::upload($request->resume, 'documents/users/'.$profile->id, null,
                                    null, false);
                                    ...
                                    return Reply::redirect(route('profile.setup'), __('menu.applicantProfile') . ' ' .
                                    __('messages.createdSuccessfully'));
                                    }
                                    ...
                                    $profile->nationality = $request->nationality;
                                    $countriesArray =
                                    json_decode(file_get_contents(public_path('country-state-city/countries.json')),
                                    true)['countries'];
                                    $statesArray =
                                    json_decode(file_get_contents(public_path('country-state-city/states.json')),
                                    true)['states'];
                                    $profile->country = $this->getName($countriesArray, $request->country);
                                    $profile->state = $this->getName($statesArray, $request->state);
                                    $profile->city = $request->city;
                                    ...
                                    return Reply::redirect(route('profile.index'), __('menu.applicantProfile') . ' ' .
                                    __('messages.createdSuccessfully'));
                                    }
                                    ...
                                    $expected_salary = str_replace(array('.', ','), '' ,
                                    $request->expected_salary_value);
                                    ...
                                    return Reply::redirect(route('profile.information'), __('menu.applicantProfile') . '
                                    ' . __('messages.createdSuccessfully'));
                                    }
                                    ...
                                    for ($i=0;$i<count($universities);$i++){ if (trim($universities[$i]) !='' &&
                                        trim($degrees[$i]) !='' && trim($studies[$i]) !='' && trim($start_dates[$i])
                                        !='' && trim($gpas[$i]) !='' && trim($is_graduateds[$i]) !='' ){ $education=new
                                        ProfileUserEducation(); if($request->has('education_id')){
                                        if($i < count($education_ids)){ if($education_ids[$i] !=null)
                                            $education=ProfileUserEducation::find($education_ids[$i]); } } $education->
                                            ...
                                            return Reply::redirect(route('profile.educations'),
                                            __('menu.applicantProfile') . ' ' . __('messages.createdSuccessfully'));
                                            }
                                            ...
                                            //create profile user skills
                                            $old_skills = ProfileUserSkill::where('profile_user_id',
                                            $profile->id)->get();
                                            foreach ($old_skills as $key => $skill) {
                                            $skill->delete();
                                            }
                                            ...
                                            for ($i=0;$i<count($skills);$i++){ if (trim($skills[$i]) !='' ){ $skill=new
                                                Skill(); if(Skill::where('name', 'LIKE' , $skills[$i])->first()){
                                                $skill = Skill::where('name', 'LIKE', $skills[$i])->first();
                                                }
                                                $skill->name = ucwords($skills[$i]);
                                                $skill->save();
                                                ...
                                                if($request->has('experience_id')){
                                                $experience_ids = $request->input('experience_id');
                                                $experiences = ProfileUserExperience::where('profile_user_id',
                                                $profile->id)->get();
                                                foreach ($experiences as $key => $experience) {
                                                if (!in_array($experience->id, $experience_ids)) {
                                                $experience->delete();
                                                ...
                                                for ($i=0;$i<count($start_periods);$i++) { if (trim($start_periods[$i])
                                                    !='' && trim($comps[$i]) !='' && trim($positions[$i]) !='' ) {
                                                    $jobExp=new ProfileUserExperience(); if($request->
                                                    has('experience_id')){
                                                    if($i < count($experience_ids)){ if($experience_ids[$i] !=null)
                                                        ...
                                                        $old_exps = ProfileUserExperience::where('profile_user_id',
                                                        $profile->id)->get();
                                                        ...
                                                        return Reply::redirect(route('profile.skills'),
                                                        __('menu.applicantProfile') . ' ' .
                                                        __('messages.createdSuccessfully'));
                                                        }
                                                        ...
                                                        if($request->has('language_id')){
                                                        $language_ids = $request->input('language_id');
                                                        $languages = ProfileUserLanguage::where('profile_user_id',
                                                        $profile->id)->get();
                                                        foreach ($languages as $key => $language) {
                                                        if (!in_array($language->id, $language_ids)) {
                                                        $language->delete();
                                                        ...
                                                        for ($i=0;$i<count($language_names);$i++) { if
                                                            (trim($language_names[$i]) !='' &&
                                                            trim($language_levels[$i]) !='' ) { $lang=new
                                                            ProfileUserLanguage(); if($request->has('language_id')){
                                                            if($i < count($language_ids)){ if($language_ids[$i] !=null)
                                                                ...
                                                                return Reply::redirect(route('profile.languages'),
                                                                __('menu.applicantProfile') . ' ' .
                                                                __('messages.createdSuccessfully'));
                                                                }
                                                                ...
                                                                $portfolio_ids = $request->input('portfolio_id');
                                                                $portfolios =
                                                                ProfileUserPortfolio::where('profile_user_id',
                                                                $profile->id)->get();
                                                                foreach ($portfolios as $key => $portfolio) {
                                                                if (!in_array($portfolio->id, $portfolio_ids)) {
                                                                $portfolio->delete();
                                                                ...
                                                                for ($i=0;$i<count($portfolios);$i++) { if
                                                                    (trim($portfolios[$i]) !='' && trim($urls[$i]) !=''
                                                                    ) { $port=new ProfileUserPortfolio(); if($request->
                                                                    has('portfolio_id')){
                                                                    if($i < count($portfolio_ids)){
                                                                        ...
                                                                        Files::deleteFile($profile->resumeDocument->hashname,
                                                                        'documents/users'.$profile->id);
                                                                        $hashname = Files::upload($request->resume,
                                                                        'documents/users/'.$profile->id, null, null,
                                                                        false);
                                                                        $profile->documents()->updateOrCreate([
                                                                        'documentable_type' => ProfileUser::class,
                                                                        'documentable_id' => $profile->id,
                                                                        ...
                                                                        return
                                                                        Reply::redirect(route('profile.portfolio'),
                                                                        __('menu.applicantProfile') . ' ' .
                                                                        __('messages.createdSuccessfully'));
                                                                        }
                                                                        ...
                                                                        {
                                                                        return view('admin.job-applications.exp',
                                                                        ['_type' => request()->type, '_id' =>
                                                                        request()->id]);
                                                                        }
                                                                        ...
                                                                        {
                                                                        $exps =
                                                                        $request->_type::find($request->_id)->experiences;
                                                                        ...
                                                                        public function fetchCountryState(Request
                                                                        $request)
                                                                        {
                                                                        $responseArr = [];
                                                                        ...
                                                                        $responseArr = Arr::add($responseArr,
                                                                        ...
                                                                        $filteredStates = array_filter($statesArray,
                                                                        function ($value) use ($countryId) {
                                                                        ...
                                                                        $response = Arr::add($response, "result",
                                                                        $responseArr);
                                                                        ...
                                                                        {
                                                                        $data = JobLocation::where('location', 'like',
                                                                        '%' . $request->q . '%')->select('id', 'location
                                                                        as text')->orderBy('location')->limit(5)->get();
                                                                        $res = array();
                                                                        foreach ($data as $d) {
                                                                        $res[] = $d->text;
                                                                        }
